...
**Sub-PR cadence.** 6 sub-PRs, each independently shippable + low-blast-radius. The first 5 build up the replacement primitives; PR-6 is the actual deletion.
  ...
  5. **PR-5 — Stop calling `getDatasetAssembled` from the cold-cache hydration path.** Scope adjusted from the original plan: dropping the `.rows` field from `CsvDataset` outright would regress 14 serializer/save/audit paths in the dashboard parent and the Schedule B Apply merge flow that reads `prev.deliveryScheduleBase?.rows` to merge incoming PDF-import deltas. Tighter PR-5: remove the `getRemoteDatasetAssembled` mutation declaration + ref, delete the assembled-batch try/catch in `loadSingleDataset`, and let cold hydration always take the per-key fallback (`getRemoteDataset` per source, chunked at `REMOTE_DATASET_CHUNK_CHAR_LIMIT` 250 KB). Eliminates the 50–150 MB single-response memory bombs that crashed Chrome tabs while preserving the in-memory `datasets[k].rows` state for the merge + signature + audit-log consumers. The `.rows` drop + the rest of the auto-sync save path moves to a follow-up workstream once those consumers are individually migrated.
  6. **PR-6 — Delete the server-side `getDatasetAssembled` procedure + the client-only `parseChunkPointerPayload` helper.** With no remaining callers (PR-5 removed the only one), remove the procedure body (~167 lines in `server/_core/solarRecDashboardRouter.ts`) and the corresponding client-side `parseChunkPointerPayload` definitions inside `SolarRecDashboard.tsx` (line 661 — the per-page copies in EarlyPayment + DeepUpdateSynthesizer stay because they parse the per-key `getDataset` response, which uses the same chunk-pointer format). Server-side `parseChunkPointerPayload` in `server/routers/helpers/scheduleB.ts` stays (Schedule B import uses it). `solarRecDashboardStorage` table stays. `_rawSourcesV1` manifest format stays.
...
**Gate after 5.14 — partially met.** The legacy chunked-CSV reassembly procedure (`getDatasetAssembled`) is gone server-side (PR-6, #175) along with its private helpers + the orphan `getSolarRecDashboardPayloadsBatch` DB helper. Cold-cache hydration always takes the per-key `getDataset` route now, capped at 250 KB per chunk — the 50–150 MB single-response memory bombs that crashed Chrome tabs in the 2026-04-26 OOM events are eliminated. The CLAUDE.md data-flow section was rewritten to drop the deprecation grandfather clause and replace it with a "removed in Task 5.14 PR-6" historical note.

**What's still in flight (deferred to a follow-up workstream).** PR-5 documented why dropping the parent's `.rows` field from `CsvDataset` outright would regress 14 serializer/save/audit paths in `SolarRecDashboard.tsx` plus the Schedule B Apply merge that reads `prev.deliveryScheduleBase?.rows` to merge incoming PDF deltas. PR-5 + PR-6 only removed the cold-load read path; the parent still hydrates `.rows` from the per-key chunked payload and auto-sync still serializes from it. The full collapse of in-memory `datasets[k].rows` state is its own task — out of scope for 5.14 because each consumer (auto-sync, signature builder, audit log, Schedule B Apply, sources merge, IndexedDB serializer) wants a different replacement and they need to be migrated one at a time. The CLAUDE.md hard rule "no **client tab** is allowed to read `datasets[key].rows`" continues to hold and is still the stronger rule that gates new code; extending it to the parent component is the follow-up workstream.

### Task 5.15 — Snapshot Log restore: orphaned chunks back into the canonical key

**Why this exists.** PRs #353/#354/#356 shipped a **read-only** recovery surface for the Snapshot Log feature. `solarRecDashboard.getSnapshotLogs` reads `snapshot_logs_v1` plus orphaned `snapshot_logs_v1_chunk_NNNN` rows in `solarRecDashboardStorage`, composes a deduped recovery candidate, and the dashboard renders it under a separate `recoveredSnapshotLogEntries` state so the cloud-sync useEffect can't accidentally write it back. Production state on 2026-05-04: the main cloud row holds a single-entry array, while ~21 unique historical entries are still pinned in orphaned chunk rows from a prior chunked write that got partially overwritten. The user can *see* the recovered entries in the UI today, but they are not persisted anywhere — a fresh browser still sees the one-entry main row, and any local create/delete/clear in another session can clobber the cloud history because the write-side guard does not exist yet.

This task closes the loop: consolidate the recovered entries back into `snapshot_logs_v1`, prune the orphan chunk rows, and add the write-side guard CLAUDE.md flagged but never wired. Two PRs, sequenced — guard first so the restore can't be silently undone by the next normal sync.

**Scope (does NOT include).** Out of scope for 5.15 — these are the larger Phase 2+ "target state" item:

  - Moving snapshot logs to a real row table (`solarRecSnapshotLogs(scopeId, id, createdAt, payloadJson)`) keyed by `scopeId`. CLAUDE.md "Snapshot Log — transitional state" calls this out as the eventual target; it stays unscheduled and out of 5.15.
  - Pagination over a server-owned data plane. The current 50-entry `getSnapshotLogs` pagination is enough for restore; the server-of-truth pagination story rides on the row-table migration.
  - Any change to the localStorage `solarRecDashboardLogsV1` shape or the `serializeDashboardLogs` / `deserializeDashboardLogs` round-trip.

**Sub-PR cadence.** 2 sub-PRs.

  1. **PR-A — Write-side guard against unsafe shrink.** The cloud-sync useEffect at `client/src/features/solar-rec/SolarRecDashboard.tsx:4808` writes `logEntries` to `REMOTE_SNAPSHOT_LOGS_KEY` whenever the local signature changes. Today nothing prevents a fresh-browser one-entry local from overwriting a 21-entry cloud history. Add a pre-write check: query `getSnapshotLogs` (or a new slim `getSnapshotLogsServerCount` if the existing proc's response is heavier than needed) for the current server unique-id count, and refuse the write when `logEntries.length < serverUniqueIdCount`. On refusal: surface a one-line storage notice ("Local snapshot log has fewer entries than cloud — sync paused. Open the Snapshot Log tab to recover.") and leave `remoteLogsSignatureRef` unchanged so the next genuine local growth re-attempts. The guard logic + threshold lives in a pure helper (`shouldSkipSnapshotLogSyncForUnsafeShrink({ localCount, serverUniqueIdCount }): boolean`) so it's unit-testable without React Query. **Definition of Done.** Unit test: a 1-entry local + 21-entry cloud refuses sync; a 21-entry local + 21-entry cloud writes through; a 22-entry local + 21-entry cloud writes through. Manual: open dashboard in fresh browser, confirm cloud row count is unchanged after dashboard hydrates and idles for 60s. tsc clean; vitest green.

  2. **PR-B — One-shot restore mutation + UI affordance.** New tRPC mutation `solarRecDashboard.restoreSnapshotLogsFromOrphanChunks` (write, gated on `solar-rec-dashboard:edit`):
       - Server re-runs `composeSnapshotLogRecovery` (existing helper in `server/services/solar/snapshotLogRecovery.ts` — pure, already tested).
       - If `recovery.source === "main"` or `"none"`, no-op and return `{ entriesRestored: 0, chunksConsolidated: 0, orphanRowsPruned: 0, alreadyConsolidated: true }`. Idempotent.
       - Else: serialize the full deduped entry list to JSON, split via the existing `splitTextIntoChunks` at `REMOTE_DATASET_CHUNK_CHAR_LIMIT = 250 KB` if needed, write the consolidated payload(s) to `snapshot_logs_v1` (and the new chunk keys, if any) using the same `solarRecDashboardStorage` upsert path the client uses. **Verify** the new row(s) deserialize cleanly to the expected unique-id set, then **delete** the orphaned `snapshot_logs_v1_chunk_NNNN` rows whose keys are NOT in the new chunk-key list. Verify-then-delete, not transactional — chunk rows are independently keyed, and a half-completed restore that wrote new content but failed to prune orphans is recoverable on retry (idempotent).
       - Returns `{ entriesRestored, chunksConsolidated, orphanRowsPruned, _runnerVersion }`.
       - Companion debug query `debugSnapshotLogPersistenceRaw` returning the main row's storageKey + payload length + every orphan-chunk row's storageKey + payload length + the recovery `source` verdict. Surfaces in the Snapshot Log tab as a "show debug" button so the operator can confirm orphan rows are gone after the restore.
     UI: `SnapshotLogTab.tsx` reads `snapshotLogsServerQuery.data.source` (already on the wire) and renders an info banner when source is `main-plus-orphaned-chunks` or `orphaned-chunks`: "N hidden snapshot entries found in cloud orphans. [Recover and consolidate]". Button calls the mutation, on success invalidates `getSnapshotLogs` and clears `recoveredSnapshotLogEntries` (the recovered entries are now in the main key; the next read will see `source === "main"`). Failure surfaces the server's `errorMessage` inline. **Definition of Done.** Run on prod-like fixture (1-entry main + 6-chunk orphan with 21 unique entries): post-mutation, main row holds 21 entries, all 6 orphan rows are absent, `getSnapshotLogs` returns `source === "main"`, the UI banner is gone. Calling the mutation a second time returns `alreadyConsolidated: true` with zero side effects. tsc clean; vitest green; new tests cover the no-op idempotent path, the consolidate path, the verify-then-delete order, and a "verify failed → no orphan delete" defensive path.

**Why this order.** PR-A is a one-line behavioral change with no schema or data effect — it can ship immediately and protects against the worst failure mode (silent shrink-overwrite by a fresh browser). PR-B is the actual restore + cleanup, riskier because it deletes rows. Shipping the guard first means a buggy PR-B can be reverted without the cloud history regressing in the meantime.

**Verification before merge (production restore is operator-driven).** PR-A merges normally. PR-B merges with `restoreSnapshotLogsFromOrphanChunks` deployed but **not auto-fired**: the operator opens the Snapshot Log tab, reads the banner, clicks the button. The mutation runs once, the orphans collapse into the main key, and the feature returns to a single-row persistence model until the Phase 2+ row-table migration replaces it entirely. Document this in the PR-B description.

**Gate after 5.15.** `getSnapshotLogs` returns `source === "main"` on prod. `solarRecDashboardStorage WHERE storageKey LIKE 'snapshot_logs_v1_chunk_%' AND payload != ''` returns zero rows for the active scope. The cloud-sync write-side guard refuses to sync when local has fewer unique ids than cloud. CLAUDE.md "Snapshot Log — transitional state" updated to drop the "currently enforced by the merge helper in the read direction only; write-side protections come with the restore PR" sentence and replace it with a "DONE in Task 5.15" pointer.
