...
Delivery Tracker unmatched-transfer diagnostics use the same path via
`startDashboardCsvExport({ exportType: "deliveryTrackerUnmatchedTransfersCsv" })`;
the tab receives bounded diagnostic previews and full bucket exports
are spooled server-side.

| Replacement proc | Wire shape | Notes |
|---|---|---|
| `solarRecDashboard.startDashboardCsvExport` | Mutation, returns `{ jobId, _runnerVersion }` (~200 B). | Enqueues a CSV export job for the ownership tile, change-ownership tile, raw dataset CSV, Delivery Tracker full detail CSV, or Delivery Tracker unmatched-transfer diagnostic CSV. The MB-scale CSV no longer crosses tRPC. |
...
baseline that started the spiral was attributed in part to
`solarRecComputedArtifacts` accumulating ≥20 × 5.7 MB `foundation-v1`
rows (PR #500's TTL prune covers it); other suspected contributors
(build runner static-input maps, `getOrBuildSystemSnapshot`'s
in-process payload, leaked `inFlightDashboardPayloadLoads` Map
entries) remain candidates pending live H-1 diagnostic capture.
