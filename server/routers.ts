...
export const appRouter = router({
  ...
  solarRecDashboard: router({
    ...
    /**
     * One-shot migration ledger repair.
     *
     * Addresses the 2026-04-10 finding: `__drizzle_migrations` shows 0011
     * as the last applied migration, but the repo journal expects through
     * 0015, and some of those migrations' DDL is partially or fully
     * missing from the DB.
     *
     * Also generates + applies a new 0016 migration that changes the
     * `monitoringApiRuns` unique key to include `connectionId`.
     *
     * This endpoint is idempotent per-statement: each DDL step checks
     * `information_schema` first and only applies what's actually missing.
     * Safe to call multiple times. Returns a structured report of what
     * was found, what was applied, and what failed.
     *
     * Gated on `adminProcedure` so only admin users can trigger it.
     * After the 2026-04-10 session this endpoint should be removed in a
     * subsequent cleanup commit once the user confirms the repair ran.
     */
    repairScheduleBMigrationLedger: adminProcedure
      .mutation(async ({ ctx }) => {
        ...
        type StepReport = {
          name: string;
          status: "skipped_already_applied" | "applied" | "failed" | "noop";
          detail?: string;
          error?: string;
        };
        const steps: StepReport[] = [];

        const record = (report: StepReport) => {
          steps.push(report);
          console.log(
            `[repairScheduleBMigrationLedger] ${report.name}: ${report.status}${report.detail ? ` — ${report.detail}` : ""}${report.error ? ` ERROR: ${report.error}` : ""}`
          );
        };
        ...
        const tableExists = async (tableName: string): Promise<boolean> => {
          ...
          `)) as unknown as Array<Array<{ cnt: number | string }>>;
          const rows = Array.isArray(result) ? result[0] : [];
          const cnt = Number((Array.isArray(rows) ? rows[0]?.cnt : 0) ?? 0);
          return cnt > 0;
        };

        // ── Helper: check if a column has a given data_type ───────
        const columnDataType = async (
          ...
          `)) as unknown as Array<Array<{ type: string }>>;
          const rows = Array.isArray(result) ? result[0] : [];
          const type = Array.isArray(rows) ? rows[0]?.type : undefined;
          return type ? String(type).toLowerCase() : null;
        };

        // ── Helper: check if a named index exists ──────────────────
        ...
        // ── Helper: check if a ledger hash is already recorded ─────
        const ledgerHasHash = async (hash: string): Promise<boolean> => {
          const result = (await db.execute(sql`
            SELECT COUNT(*) AS cnt
        ...
        // ── Helper: record a migration in the ledger ──────────────
        const insertLedgerRow = async (hash: string, createdAt: number) => {
        ...
        // ── Helper: read a migration .sql file from disk ───────────
        // In production (esbuild bundle) the drizzle folder is typically
        // shipped alongside the server build. Try a few candidate paths.
        const resolveMigrationFolder = (): string | null => {
          const candidates = [
            nodePath.resolve(process.cwd(), "drizzle"),
            nodePath.resolve(process.cwd(), "productivity-hub", "drizzle"),
            nodePath.resolve(
              nodePath.dirname(
          ...
          for (const candidate of candidates) {
            try {
              if (existsSync(nodePath.join(candidate, "meta", "_journal.json"))) {
                return candidate;
              }
            } catch {
              // continue
        ...
        const readMigrationSource = (tag: string): string | null => {
          if (!migrationFolder) return null;
          const filePath = nodePath.join(migrationFolder, `${tag}.sql`);
          try {
        ...
        const computeHash = (source: string): string =>
          crypto.createHash("sha256").update(source).digest("hex");

        // These numbers come directly from drizzle/meta/_journal.json
        const JOURNAL_WHENS: Record<string, number> = {
          "0012_volatile_boomerang": 1774718170219,
          "0013_jittery_viper": 1775351678821,
          "0014_curly_lockjaw": 1775414911332,
          "0015_useful_blonde_phantom": 1775521137222,
        };

        // ──────────────────────────────────────────────────────────
        // 0012: productionReadings table
        // ──────────────────────────────────────────────────────────
        try {
          if (await tableExists("productionReadings")) {
            record({
              name: "0012.productionReadings",
            ...
            `);
            await db.execute(
              sql`CREATE INDEX \`production_readings_email_idx\` ON \`productionReadings\` (\`customerEmail\`)`
            );
            await db.execute(
              sql`CREATE INDEX \`production_readings_nonid_idx\` ON \`productionReadings\` (\`nonId\`)`
            );
            await db.execute(
              sql`CREATE INDEX \`production_readings_read_at_idx\` ON \`productionReadings\` (\`readAt\`)`
            );
            record({
              name: "0012.productionReadings",
              status: "applied",
              detail: "table and 3 indexes created",
            });
          }
        } catch (err) {
          record({
        ...
        // ──────────────────────────────────────────────────────────
        // 0013: integrations.metadata → mediumtext
        // ──────────────────────────────────────────────────────────
        try {
          const currentType = await columnDataType("integrations", "metadata");
          if (currentType === "mediumtext") {
            record({
              name: "0013.integrations.metadata",
              status: "skipped_already_applied",
              detail: "column is already mediumtext",
            });
          } else if (currentType === null) {
            record({
              name: "0013.integrations.metadata",
              status: "failed",
              error: "integrations.metadata column not found",
            });
          } else {
            await db.execute(
              sql`ALTER TABLE \`integrations\` MODIFY COLUMN \`metadata\` mediumtext`
            );
            record({
              name: "0013.integrations.metadata",
              status: "applied",
              detail: `was ${currentType} → mediumtext`,
            });
          }
        } catch (err) {
          record({
        ...
        // ──────────────────────────────────────────────────────────
        // 0014: supplementPriceLogs table
        // ──────────────────────────────────────────────────────────
        try {
          if (await tableExists("supplementPriceLogs")) {
            record({
              name: "0014.supplementPriceLogs",
              status: "skipped_already_applied",
              detail: "table already exists",
            });
          } else {
            ...
            `);
            await db.execute(
              sql`CREATE INDEX \`supplement_price_logs_user_captured_idx\` ON \`supplementPriceLogs\` (\`userId\`,\`capturedAt\`)`
            );
            await db.execute(
              sql`CREATE INDEX \`supplement_price_logs_user_definition_captured_idx\` ON \`supplementPriceLogs\` (\`userId\`,\`definitionId\`,\`capturedAt\`)`
            );
            record({
              name: "0014.supplementPriceLogs",
              status: "applied",
              detail: "table and 2 indexes created",
            });
          }
        } catch (err) {
          record({
        ...
        // ──────────────────────────────────────────────────────────
        // 0015: monitoring + solar-rec tables. Each CREATE is wrapped
        // in an independent try so a partial pre-existing state (e.g.
        // monitoringApiRuns already exists) doesn't abort the rest.
        // ──────────────────────────────────────────────────────────
        const create0015Table = async (
          name: string,
          createDdl: (db: typeof ctx) => Promise<void>
        ) => {
          try {
        ...
        await create0015Table("monitoringApiRuns", async () => {
          await db.execute(sql`
          ...
          `);
          await db.execute(
            sql`CREATE INDEX \`monitoring_api_runs_date_key_idx\` ON \`monitoringApiRuns\` (\`dateKey\`)`
          );
          await db.execute(
            sql`CREATE INDEX \`monitoring_api_runs_provider_date_idx\` ON \`monitoringApiRuns\` (\`provider\`,\`dateKey\`)`
          );
          await db.execute(
            sql`CREATE INDEX \`monitoring_api_runs_status_date_idx\` ON \`monitoringApiRuns\` (\`status\`,\`dateKey\`)`
          );
        });

        await create0015Table("monitoringBatchRuns", async () => {
          await db.execute(sql`
          ...
          `);
          await db.execute(
            sql`CREATE INDEX \`monitoring_batch_runs_date_key_idx\` ON \`monitoringBatchRuns\` (\`dateKey\`)`
          );
          await db.execute(
            sql`CREATE INDEX \`monitoring_batch_runs_status_idx\` ON \`monitoringBatchRuns\` (\`status\`)`
          );
        });

        await create0015Table("solarRecInvites", async () => {
          await db.execute(sql`
          ...
          `);
          await db.execute(
            sql`CREATE INDEX \`solar_rec_invites_email_idx\` ON \`solarRecInvites\` (\`email\`)`
          );
        });

        await create0015Table("solarRecTeamCredentials", async () => {
          await db.execute(sql`
          ...
          `);
          await db.execute(
            sql`CREATE INDEX \`solar_rec_team_credentials_provider_idx\` ON \`solarRecTeamCredentials\` (\`provider\`)`
          );
        });

        await create0015Table("solarRecUsers", async () => {
          await db.execute(sql`
          ...
          `);
          await db.execute(
            sql`CREATE INDEX \`solar_rec_users_email_idx\` ON \`solarRecUsers\` (\`email\`)`
          );
          await db.execute(
            sql`CREATE INDEX \`solar_rec_users_google_open_id_idx\` ON \`solarRecUsers\` (\`googleOpenId\`)`
          );
        });

        // ──────────────────────────────────────────────────────────
        // Ledger rows for 0012-0015. Only insert if not already
        // present. Hashes are computed from the .sql files shipped
        // with the repo; if the files aren't bundled with the
        // deployed server, fall back to a synthetic marker hash so
        // drizzle-kit at least sees a ledger entry for the correct
        // folderMillis value. The synthetic marker is namespaced so
        // we can identify it later.
        // ──────────────────────────────────────────────────────────
        const recordLedger = async (tag: string) => {
          try {
            const when = JOURNAL_WHENS[tag];
            if (!when) {
              record({
                name: `ledger.${tag}`,
                status: "failed",
                error: "no journal entry for tag",
              });
              return;
            }
            const source = readMigrationSource(tag);
            const hash = source
              ? computeHash(source)
              : `synthetic:repair-2026-04-10:${tag}`;
            if (await ledgerHasHash(hash)) {
              record({
                name: `ledger.${tag}`,
                status: "skipped_already_applied",
                detail: `hash ${hash.slice(0, 12)}… already in __drizzle_migrations`,
              });
              return;
            }
            await insertLedgerRow(hash, when);
            record({
              name: `ledger.${tag}`,
              status: "applied",
              detail: `inserted hash ${hash.slice(0, 12)}… created_at=${when}${source ? "" : " (synthetic)"}`,
            });
          } catch (err) {
            record({
              name: `ledger.${tag}`,
        ...
        await recordLedger("0012_volatile_boomerang");
        await recordLedger("0013_jittery_viper");
        await recordLedger("0014_curly_lockjaw");
        await recordLedger("0015_useful_blonde_phantom");

        // ──────────────────────────────────────────────────────────
        // 0016: monitoringApiRuns unique index now includes connectionId.
        // Requires deduplication first (keep the MIN(id) per group).
        // ──────────────────────────────────────────────────────────
        try {
          if (!(await tableExists("monitoringApiRuns"))) {
            record({
              name: "0016.monitoringApiRuns.precheck",
              status: "failed",
              error: "monitoringApiRuns table does not exist",
            });
          } else {
            const oldIndexName = "monitoring_api_runs_provider_site_date_idx";
            const newIndexName = "monitoring_api_runs_provider_conn_site_date_idx";
            const newIndexExists = await indexExists("monitoringApiRuns", newIndexName);
            const oldIndexExists = await indexExists("monitoringApiRuns", oldIndexName);

            if (newIndexExists && !oldIndexExists) {
              record({
                name: "0016.monitoringApiRuns.uniqueKey",
                status: "skipped_already_applied",
                detail: "new unique index already in place",
              });
            } else {
              // Deduplicate first, in case the old unique key tolerated
              // duplicates under the new key definition. Keep MIN(id) per
              // (provider, siteId, dateKey) group — any conflict under the
              // new (provider, connectionId, siteId, dateKey) key will
              ...
              const dedupResult = (await db.execute(sql`
                ...
                INNER JOIN (
                  ...
                  ON keep.provider = m.provider
                 AND keep.siteId = m.siteId
                 AND keep.dateKey = m.dateKey
                 AND m.id <> keep.keep_id
              `)) as unknown as { affectedRows?: number };
              const dedupedCount =
                typeof (dedupResult as { affectedRows?: number }).affectedRows ===
                "number"
                  ? (dedupResult as { affectedRows?: number }).affectedRows
                  : null;

              if (oldIndexExists) {
                await db.execute(
                  sql.raw(
                    `ALTER TABLE \`monitoringApiRuns\` DROP INDEX \`${oldIndexName}\``
                  )
                );
              }
              ...
              await db.execute(
                sql.raw(
                  `CREATE UNIQUE INDEX \`${newIndexName}\` ON \`monitoringApiRuns\` (\`provider\`,\`connectionId\`,\`siteId\`,\`dateKey\`)`
                )
              );
              ...
              record({
                name: "0016.monitoringApiRuns.uniqueKey",
                status: "applied",
                detail: `deduplicated ${dedupedCount ?? "unknown"} rows, dropped old index, created new unique index`,
              });

              // Synthetic ledger entry for 0016 since the .sql file
              // hasn't been generated yet. Use a namespaced hash so a
              // future drizzle-kit generate + migrate run can identify
              // and replace it.
              const syntheticHash =
                "synthetic:repair-2026-04-10:0016_monitoring_uniqueness_connectionId";
              const syntheticWhen = Date.now();
              if (!(await ledgerHasHash(syntheticHash))) {
                await insertLedgerRow(syntheticHash, syntheticWhen);
                record({
                  name: "ledger.0016_monitoring_uniqueness",
                  status: "applied",
                  detail: `inserted synthetic ledger row (created_at=${syntheticWhen})`,
                });
              } else {
                record({
                  name: "ledger.0016_monitoring_uniqueness",
                  status: "skipped_already_applied",
                  detail: "synthetic ledger row already present",
                });
              }
            }
          }
        ...
        const summary = {
          ...
          applied: steps.filter((s) => s.status === "applied").length,
          skipped: steps.filter((s) => s.status === "skipped_already_applied")
            .length,
          failed: steps.filter((s) => s.status === "failed").length,
        };

        return {
          _runnerVersion: "repair-v1-2026-04-10",
          summary,
          steps,
          userId: ctx.user.id,
        };
