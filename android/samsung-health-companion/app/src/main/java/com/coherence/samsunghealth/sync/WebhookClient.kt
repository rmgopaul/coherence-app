 ...
 *
 * Copied verbatim in spirit from the healthconnect-companion
 * module's [WebhookResult] so the sync semantics match.
 */
 ...
 *
 * Posts a single-day [SamsungHealthPayload] to
 * `POST /api/webhooks/samsung-health` with the `x-sync-key` header,
 * exactly like the healthconnect-companion module.
 */
class WebhookClient(
