...
export default function APsystemsMeterReads() {
      ...
      if (bulkCancelRef.current) {
        ...
        try {
          ...
          if (readRows.length === 0) {
            toast.message(
              `No APSystems rows to push to Converted Reads — ${NUMBER_FORMATTER.format(found)} sites returned but none had a lifetime kWh reading.`
            );
          } else {
            ...
            if (result.pushed > 0) {
              toast.success(
                `Pushed ${NUMBER_FORMATTER.format(result.pushed)} APSystems rows to Solar REC Dashboard Converted Reads.${result.skipped > 0 ? ` ${NUMBER_FORMATTER.format(result.skipped)} duplicates skipped.` : ""}`
              );
            } else if (result.skipped > 0) {
              toast.message(
                `All ${NUMBER_FORMATTER.format(result.skipped)} APSystems Converted Reads rows already exist. No new rows pushed.`
              );
            } else {
              toast.message("APSystems Converted Reads push returned 0 rows.");
            }
          }
        } catch (pushError) {
  ...
  const downloadConvertedReadsCsv = (rows: BulkSnapshotRow[]) => {
    ...
    for (const row of readRows) {
      const base = buildConvertedReadRow(MONITORING_CANONICAL_NAMES.apsystems, row.systemId, row.name ?? "", row.lifetimeKwh!, row.anchorDate!);
