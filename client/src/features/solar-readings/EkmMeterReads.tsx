...
type SingleOperation = "getProductionSnapshot";
...
function extractMeterNumbersFromCsv(text: string): string[] {
  ...
  const preferredIndex = normalizedHeaders.findIndex((header) =>
    ["meternumber", "meter_number", "meter", "id"].includes(header)
  );
  ...
  if (preferredIndex >= 0) {
    const meterHeader = parsed.headers[preferredIndex];
    return Array.from(
      new Set(
        parsed.rows
          .map((row) => clean(row[meterHeader]))
...
export default function EkmMeterReads() {
  ...
  const [bulkMeterNumbers, setBulkMeterNumbers] = useState<string[]>([]);
  ...
  const [bulkSort, setBulkSort] = useState<BulkSortKey>("meterNumber");
  ...
  const connectMutation = trpc.ekm.connect.useMutation();
  const setActiveConnectionMutation = trpc.ekm.setActiveConnection.useMutation();
  const removeConnectionMutation = trpc.ekm.removeConnection.useMutation();
  const disconnectMutation = trpc.ekm.disconnect.useMutation();
  const productionSnapshotMutation = trpc.ekm.getProductionSnapshot.useMutation();
  ...
  useEffect(() => {
    setBulkRows([]);
    setBulkProgress({ total: bulkMeterNumbers.length, processed: 0, found: 0, notFound: 0, errored: 0 });
  }, [bulkMeterNumbers.length]);
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      await trpcUtils.ekm.getStatus.invalidate();
      setSelectedConnectionId(response.activeConnectionId);
      setApiKeyInput("");
      setConnectionNameInput("");
      toast.success(
        `EKM profile saved. ${NUMBER_FORMATTER.format(response.totalConnections)} profile(s) stored.`
      );
    } catch (error) {
  ...
  const handleSetActiveConnection = async () => {
    ...
    try {
      ...
      await trpcUtils.ekm.getStatus.invalidate();
      toast.success("Active EKM API profile updated.");
    } catch (error) {
    ...
    try {
      ...
      toast.success(
        response.connected
          ...
          : "Removed final profile. EKM is now disconnected."
      );
    } catch (error) {
  ...
  const handleDisconnect = async () => {
    try {
      ...
      await trpcUtils.ekm.getStatus.invalidate();
      setSelectedConnectionId("");
      toast.success("EKM disconnected.");
    } catch (error) {
    ...
    switch (selectedOperation) {
      case "getProductionSnapshot":
        if (!selectedMeterNumber) {
          toast.error("Enter a Meter Number first.");
          return;
        }
  ...
  const handleBulkFileUpload = async (file: File | null) => {
    ...
    try {
      ...
      const meterNumbers = extractMeterNumbersFromCsv(raw);
      if (meterNumbers.length === 0) {
        setBulkImportError("No valid Meter Numbers found in CSV.");
        setBulkMeterNumbers([]);
      ...
      setBulkSourceFileName(file.name);
      setBulkMeterNumbers(meterNumbers);
      setBulkRows([]);
      setBulkProgress({ total: meterNumbers.length, processed: 0, found: 0, notFound: 0, errored: 0 });
      toast.success(`Imported ${NUMBER_FORMATTER.format(meterNumbers.length)} Meter Numbers.`);
    } catch (error) {
  ...
  const runBulkSnapshot = async () => {
    if (!statusQuery.data?.connected) {
      toast.error("Connect EKM before running bulk processing.");
      return;
    }
    if (bulkMeterNumbers.length === 0) {
      toast.error("Upload a CSV with Meter Numbers first.");
      return;
    }
    ...
    const chunks = chunkArray(bulkMeterNumbers, effectiveBatchSize);
    let processed = 0;
    let found = 0;
    let notFound = 0;
    let errored = 0;
    ...
    try {
      for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex += 1) {
        ...
        try {
          const raw = await productionSnapshotMutation.mutateAsync({
            meterNumber: chunk[0],
            anchorDate: bulkAnchorDate,
          });

          // Process each meter number individually since EKM has no batch endpoint
          const batchRows: BulkSnapshotRow[] = [];
          for (const meterNumber of chunk) {
            if (bulkCancelRef.current) break;
            try {
              ...
              const errorMessage = toErrorMessage(meterError);
      ...
      if (bulkCancelRef.current) {
        toast.message(
          `Stopped production snapshots after ${NUMBER_FORMATTER.format(processed)} of ${NUMBER_FORMATTER.format(bulkMeterNumbers.length)} Meter Numbers.`
        );
      } else {
        toast.success(
          `Completed production snapshots for ${NUMBER_FORMATTER.format(processed)} Meter Numbers. Found ${NUMBER_FORMATTER.format(found)}, not found ${NUMBER_FORMATTER.format(notFound)}, errors ${NUMBER_FORMATTER.format(errored)}.`
        );
        ...
        try {
          const readRows = collectedRows
            ...
            .map((row) =>
              buildConvertedReadRow("EKM", row.meterNumber, row.name ?? "", row.lifetimeKwh!, row.anchorDate!)
            );
          ...
          if (result.pushed > 0) {
            toast.success(`Pushed ${NUMBER_FORMATTER.format(result.pushed)} EKM rows to Solar REC Dashboard Converted Reads.${result.skipped > 0 ? ` ${NUMBER_FORMATTER.format(result.skipped)} duplicates skipped.` : ""}`);
          } else if (result.skipped > 0) {
            toast.message(`All ${NUMBER_FORMATTER.format(result.skipped)} EKM Converted Reads rows already exist. No new rows pushed.`);
          }
        } catch (pushError) {
  ...
  const filteredBulkRows = useMemo(() => {
    ...
    const filtered = bulkRows.filter((row) => {
      ...
      const haystack = `${row.meterNumber} ${row.name ?? ""} ${row.status} ${row.error ?? ""}`.toLowerCase();
  ...
  const bulkSortOptions: Array<{ value: BulkSortKey; label: string }> = [
    { value: "meterNumber", label: "Meter Number (A-Z)" },
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-accent/20">
      <header className="border-b bg-card/90 backdrop-blur-sm sticky top-0 z-10">
        <div className="container mx-auto px-4 py-4">
          ...
          <h1 className="text-2xl font-bold text-foreground">EKM Metering API</h1>
      ...
      <main className="container mx-auto px-4 py-8 space-y-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>1) Connect EKM</CardTitle>
            <CardDescription>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="ekm-connection-name">Profile Name (optional)</Label>
                <Input
                  id="ekm-connection-name"
                  value={connectionNameInput}
                  onChange={(e) => setConnectionNameInput(e.target.value)}
                  placeholder="Example: EKM API 1"
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="ekm-api-key">API Key</Label>
                <Input
                  id="ekm-api-key"
                  type="password"
                  value={apiKeyInput}
        ...
        {/* Section 2: Single Meter API Tester */}
        <Card>
          <CardHeader>
            <CardTitle>2) Single Meter API Tester</CardTitle>
            <CardDescription>
              Enter a Meter Number manually, then fetch production snapshot.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                <Label htmlFor="manual-meter-number">Meter Number</Label>
                <Input
                  id="manual-meter-number"
                  value={selectedMeterNumber}
                  onChange={(e) => setSelectedMeterNumber(e.target.value.trim())}
                  placeholder="Enter EKM Meter Number"
                />
              </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Upload a CSV of Meter Numbers and process production snapshots in batches.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Use <span className="font-medium">All Saved API Profiles</span> to check each Meter Number against every connected API.
                </p>
              </div>
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    disabled={bulkMeterNumbers.length === 0}
                    onClick={() => {
                      setBulkMeterNumbers([]);
                      setBulkRows([]);
                ...
                <p className="text-xs text-muted-foreground">
                  Source: {bulkSourceFileName ?? "None"}{bulkMeterNumbers.length > 0 ? ` — ${NUMBER_FORMATTER.format(bulkMeterNumbers.length)} IDs` : ""}
                </p>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <Button onClick={runBulkSnapshot} disabled={bulkIsRunning || bulkMeterNumbers.length === 0 || !isConnected}>
                {bulkIsRunning ? (
                  <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                ) : (
                  <Upload className="h-4 w-4 mr-2" />
              ...
              <Button
                ...
                onClick={() => downloadBulkCsv(bulkRows, "ekm-production-bulk-all")}
              >
                Download All CSV
              </Button>
              <Button
                ...
                onClick={() => downloadBulkCsv(filteredBulkRows, "ekm-production-bulk-filtered")}
              >
            ...
            <div className="rounded-lg border bg-card p-3 space-y-2">
              <div className="flex items-center justify-between text-xs text-muted-foreground">
                <span>
                  Progress: {NUMBER_FORMATTER.format(bulkProgress.processed)} / {NUMBER_FORMATTER.format(bulkProgress.total)} Meter Numbers
                </span>
            ...
            <div className="grid gap-3 md:grid-cols-5">
              <div className="rounded-lg border bg-card p-3">
                <p className="text-xs text-muted-foreground">Imported Meter Numbers</p>
                <p className="text-xl font-semibold text-foreground">{NUMBER_FORMATTER.format(bulkMeterNumbers.length)}</p>
              </div>
              <div className="rounded-lg border bg-card p-3">
            ...
            <div className="grid gap-3 md:grid-cols-3">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="Filter by Meter Number or error"
                />
              </div>
              <div className="space-y-2">
                <Label>Status</Label>
            ...
            <Table>
              ...
              <TableBody>
                {bulkPageRows.map((row) => (
                  <TableRow key={row.meterNumber}>
                    <TableCell className="font-medium font-mono text-xs">{row.meterNumber}</TableCell>
