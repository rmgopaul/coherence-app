...
// Task 5.9 PR-A (2026-04-27): ABP Invoice Settlement migrated to the
// standalone Solar REC router. `abpSettlement.*` and `csgPortal.*`
// procs now live on /solar-rec/api/trpc; `solarRecDashboard.*` was
// already there from Task 5.5. Aliased import keeps every `trpc.*`
// call site routable through the standalone tRPC client. The
// previously-separate `solarRecTrpc` reference (used only for
// solarRecDashboard) is no longer needed since `trpc` IS now
// `solarRecTrpc`.
