/**
 * Task 5.4 vendor 4/16 — Hoymiles (S-Miles Cloud) on solar-rec.
 *
 * Same template as Generac / Solis / GoodWe. Team credential stores
 * `{username, password, baseUrl}`. Credential lifecycle lives in Solar
...
export default function HoymilesMeterReads() {
  ...
  const listStationsQuery = trpc.hoymiles.listStations.useQuery(undefined, {
    enabled: statusQuery.data?.connected === true,
    retry: false,
  });
  const snapshotMutation = trpc.hoymiles.getProductionSnapshot.useMutation({
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-4">
      <div>
        <h1 className="text-xl font-semibold">Hoymiles S-Miles</h1>
        <p className="text-sm text-muted-foreground">
          Run meter reads against the team&rsquo;s Hoymiles credential. Manage
          username + password in Solar REC Settings → Credentials.
        </p>
      </div>
        ...
        <CardHeader>
          ...
          <CardDescription>
            {statusQuery.data?.connected
              ...
              : "Ask an admin to register a Hoymiles username + password in Settings → Credentials."}
          </CardDescription>
        </CardHeader>
      </Card>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <CardDescription>
                Discover stations on the connected S-Miles account.
              </CardDescription>
            </div>
            <Button
              variant="outline"
        ...
        <CardContent>
          {!statusQuery.data?.connected ? (
            ...
            <p className="text-sm text-muted-foreground">
              No stations returned by Hoymiles.
            </p>
          ) : (
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div className="space-y-1">
              <Label htmlFor="hoymiles-anchor-date">
                Anchor date (optional)
              </Label>
              <Input
                id="hoymiles-anchor-date"
                type="date"
                value={anchorDate}
