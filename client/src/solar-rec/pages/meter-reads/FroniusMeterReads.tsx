/**
 * Task 5.4 vendor 10/16 — Fronius (Solar.web) meter-reads on solar-rec.
 *
 * Team credential stores `{accessKeyId, accessKeyValue, baseUrl}`.
...
export default function FroniusMeterReads() {
  ...
  const listSystemsQuery = trpc.fronius.listPvSystems.useQuery(undefined, {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-4">
      <div>
        <h1 className="text-xl font-semibold">Fronius Solar.web</h1>
        <p className="text-sm text-muted-foreground">
          Run meter reads against the team&rsquo;s Fronius credential. Manage
          accessKeyId + accessKeyValue in Solar REC Settings → Credentials.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between gap-3">
            <div>
              <CardTitle className="text-base">PV Systems</CardTitle>
              <CardDescription>
                Discover PV systems on the connected Solar.web account.
              </CardDescription>
            </div>
            <Button
              variant="outline"
        ...
        <CardContent>
          {!statusQuery.data?.connected ? (
          ...
          ) : (listSystemsQuery.data?.pvSystems ?? []).length === 0 ? (
            <p className="text-sm text-muted-foreground">
              No PV systems returned by Solar.web.
            </p>
          ) : (
            <div className="overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {listSystemsQuery.data?.pvSystems.map((s) => (
                    <TableRow key={s.pvSystemId}>
        ...
        <CardContent className="space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="space-y-1">
              <Label htmlFor="fronius-pv-system-id">PV System ID</Label>
              <Input
                id="fronius-pv-system-id"
                value={pvSystemId}
                onChange={(e) => setPvSystemId(e.target.value)}
                placeholder="e.g., abc123-xyz-..."
              />
            </div>
            <div className="space-y-1">
              <Label htmlFor="fronius-anchor-date">
                Anchor date (optional)
              </Label>
              <Input
                id="fronius-anchor-date"
                type="date"
                value={anchorDate}
          ...
          {result && (result as any).status === "Found" && (result as any).lifetimeKwh != null && showPersist && (
            <PersistConfirmation
              ...
              rows={[{
                ...
                monitoring_system_name: String((result as any).name || (result as any).systemName || pvSystemId),
