...
export function buildDeliveryTrackerData(input: {
  ...
  // Hydration guard: if transfers are present but schedules haven't
  // arrived yet, the main loop would mark every transfer as unmatched
  // because systemSchedules is empty. Return empty until Schedule B
  // lands. The inverse (schedules alone) is a legitimate state —
  // contracts are derivable from Schedule B without transfers.
