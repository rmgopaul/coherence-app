...
function extractSystemIdsFromCsv(text: string): string[] {
  ...
  const preferredIndex = normalizedHeaders.findIndex((header) =>
    ["systemid", "system_id", "id"].includes(header)
  );
...
export default function GeneracMeterReads() {
  ...
  const systemsQuery = trpc.generac.listSystems.useQuery(undefined, {
    enabled: !!user && !!statusQuery.data?.connected,
    retry: false,
  });

  const connectMutation = trpc.generac.connect.useMutation();
  const setActiveConnectionMutation = trpc.generac.setActiveConnection.useMutation();
  const removeConnectionMutation = trpc.generac.removeConnection.useMutation();
  const disconnectMutation = trpc.generac.disconnect.useMutation();
  const productionSnapshotMutation = trpc.generac.getProductionSnapshot.useMutation();
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      await trpcUtils.generac.getStatus.invalidate();
      await trpcUtils.generac.listSystems.invalidate();
      setSelectedConnectionId(response.activeConnectionId);
      setApiKeyInput("");
      setConnectionNameInput("");
      toast.success(
        `Generac profile saved. ${NUMBER_FORMATTER.format(response.totalConnections)} profile(s) stored.`
      );
    } catch (error) {
  ...
  const handleSetActiveConnection = async () => {
    ...
    try {
      ...
      await trpcUtils.generac.listSystems.invalidate();
      setSelectedSystemId("");
      toast.success("Active Generac API profile updated.");
    } catch (error) {
    ...
    try {
      ...
      toast.success(
        response.connected
          ...
          : "Removed final profile. Generac is now disconnected."
      );
    } catch (error) {
  ...
  const handleDisconnect = async () => {
    try {
      ...
      setSelectedSystemId("");
      setSelectedConnectionId("");
      toast.success("Generac disconnected.");
    } catch (error) {
  ...
  const handlePullAllSites = async () => {
    if (!isConnected) {
      toast.error("Connect Generac before pulling sites.");
      return;
    }
    try {
  ...
  const runBulkSnapshot = async () => {
    if (!statusQuery.data?.connected) {
      toast.error("Connect Generac before running bulk processing.");
      return;
    }
      ...
      if (bulkCancelRef.current) {
        ...
        try {
          const readRows = collectedRows
            ...
            .map((row) =>
              buildConvertedReadRow("Generac", row.systemId, row.name ?? "", row.lifetimeKwh!, row.anchorDate!)
            );
          ...
          if (result.pushed > 0) {
            toast.success(`Pushed ${NUMBER_FORMATTER.format(result.pushed)} Generac rows to Solar REC Dashboard Converted Reads.${result.skipped > 0 ? ` ${NUMBER_FORMATTER.format(result.skipped)} duplicates skipped.` : ""}`);
          } else if (result.skipped > 0) {
            toast.message(`All ${NUMBER_FORMATTER.format(result.skipped)} Generac Converted Reads rows already exist. No new rows pushed.`);
          }
        } catch (pushError) {
  ...
  const downloadConvertedReadsCsv = (rows: BulkSnapshotRow[]) => {
    ...
    const csvRows = readRows.map((row) => {
      const readRow = buildConvertedReadRow("Generac PWRfleet", row.systemId, row.name ?? "", row.lifetimeKwh!, row.anchorDate!);
    ...
    const csvText = buildCsv(headers, csvRows);
    const fileName = `generac-converted-reads-${new Date().toISOString().slice(0, 19).replace(/[:T]/g, "-")}.csv`;
    downloadTextFile(fileName, csvText, "text/csv;charset=utf-8");
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-accent/20">
      <header className="border-b bg-card/90 backdrop-blur-sm sticky top-0 z-10">
        <div className="container mx-auto px-4 py-4">
          ...
          <h1 className="text-2xl font-bold text-foreground">Generac PWRview API</h1>
          <p className="text-sm text-muted-foreground mt-1">
            API Key connection for Generac PWRview monitoring endpoints, including bulk CSV processing for systems.
          </p>
        </div>
      </header>
      ...
      <main className="container mx-auto px-4 py-8 space-y-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>1) Connect Generac PWRview</CardTitle>
            <CardDescription>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="generac-connection-name">Profile Name (optional)</Label>
                <Input
                  id="generac-connection-name"
                  value={connectionNameInput}
                  onChange={(e) => setConnectionNameInput(e.target.value)}
                  placeholder="Example: Generac API 1"
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="generac-api-key">API Key</Label>
                <Input
                  id="generac-api-key"
                  type="password"
                  value={apiKeyInput}
                  onChange={(e) => setApiKeyInput(e.target.value)}
                  placeholder="Generac PWRview API Key"
                />
              </div>
            </div>
          ...
          <CardContent className="space-y-4">
            ...
            {!systemsQuery.isLoading && !systemsError && isConnected && systems.length === 0 && (
              <div className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-700 dark:border-amber-700 dark:bg-amber-950 dark:text-amber-300">
                No systems were returned for this API key.
              </div>
            )}
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Button
                ...
                onClick={() => downloadBulkCsv(bulkRows, "generac-production-bulk-all")}
              >
                Download All CSV
              </Button>
              <Button
                ...
                onClick={() => downloadBulkCsv(filteredBulkRows, "generac-production-bulk-filtered")}
              >
