...
function extractStationIdsFromCsv(text: string): string[] {
  ...
  const preferredIndex = normalizedHeaders.findIndex((header) =>
    ["stationid", "station_id", "powerstation_id", "id"].includes(header)
  );
...
export default function GoodWeMeterReads() {
  ...
  const stationsQuery = trpc.goodwe.listStations.useQuery(undefined, {
    enabled: !!user && !!statusQuery.data?.connected,
    retry: false,
  });

  const connectMutation = trpc.goodwe.connect.useMutation();
  const setActiveConnectionMutation = trpc.goodwe.setActiveConnection.useMutation();
  const removeConnectionMutation = trpc.goodwe.removeConnection.useMutation();
  const disconnectMutation = trpc.goodwe.disconnect.useMutation();
  const productionSnapshotMutation = trpc.goodwe.getProductionSnapshot.useMutation();
  ...
  const handleConnect = async () => {
    ...
    if (!account || !password) {
      toast.error("Enter both Account and Password.");
      return;
    }
    ...
    try {
      ...
      await trpcUtils.goodwe.getStatus.invalidate();
      await trpcUtils.goodwe.listStations.invalidate();
      setSelectedConnectionId(response.activeConnectionId);
      setAccountInput("");
      setPasswordInput("");
      setConnectionNameInput("");
      toast.success(
        `GoodWe profile saved. ${NUMBER_FORMATTER.format(response.totalConnections)} profile(s) stored.`
      );
    } catch (error) {
  ...
  const handleSetActiveConnection = async () => {
    ...
    try {
      ...
      toast.success("Active GoodWe API profile updated.");
    } catch (error) {
    ...
    try {
      ...
      toast.success(
        response.connected
          ...
          : "Removed final profile. GoodWe is now disconnected."
      );
    } catch (error) {
  ...
  const handleDisconnect = async () => {
    try {
      ...
      setSelectedStationId("");
      setSelectedConnectionId("");
      toast.success("GoodWe disconnected.");
    } catch (error) {
  ...
  const handlePullAllSites = async () => {
    if (!isConnected) {
      toast.error("Connect GoodWe before pulling sites.");
      return;
    }
    try {
  ...
  const runBulkSnapshot = async () => {
    if (!statusQuery.data?.connected) {
      toast.error("Connect GoodWe before running bulk processing.");
      return;
    }
    ...
    try {
      ...
      if (bulkCancelRef.current) {
        ...
        toast.success(
          `Completed production snapshots for ${NUMBER_FORMATTER.format(processed)} Station IDs using ${bulkConnectionScope === "all" ? "all saved API profiles" : "active API profile"}. Found ${NUMBER_FORMATTER.format(found)}, not found ${NUMBER_FORMATTER.format(notFound)}, errors ${NUMBER_FORMATTER.format(errored)}.`
        );
        ...
        try {
          const readRows = collectedRows
            ...
            .map((row) =>
              buildConvertedReadRow("GoodWe SEMS", row.stationId, row.name ?? "", row.lifetimeKwh!, row.anchorDate!)
            );
          ...
          if (result.pushed > 0) {
            toast.success(`Pushed ${NUMBER_FORMATTER.format(result.pushed)} GoodWe rows to Solar REC Dashboard Converted Reads.${result.skipped > 0 ? ` ${NUMBER_FORMATTER.format(result.skipped)} duplicates skipped.` : ""}`);
          } else if (result.skipped > 0) {
            toast.message(`All ${NUMBER_FORMATTER.format(result.skipped)} GoodWe Converted Reads rows already exist. No new rows pushed.`);
          }
        } catch (pushError) {
  ...
  const downloadConvertedReadsCsv = (rows: BulkSnapshotRow[]) => {
    ...
    const csvRows = readRows.map((row) => {
      const readRow = buildConvertedReadRow("GoodWe SEMS Portal", row.stationId, row.name ?? "", row.lifetimeKwh!, row.anchorDate!);
    ...
    const csvText = buildCsv(headers, csvRows);
    const fileName = `goodwe-converted-reads-${new Date().toISOString().slice(0, 19).replace(/[:T]/g, "-")}.csv`;
    downloadTextFile(fileName, csvText, "text/csv;charset=utf-8");
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-accent/20">
      <header className="border-b bg-card/90 backdrop-blur-sm sticky top-0 z-10">
        <div className="container mx-auto px-4 py-4">
          ...
          <h1 className="text-2xl font-bold text-foreground">GoodWe SEMS API</h1>
          <p className="text-sm text-muted-foreground mt-1">
            Account + Password connection for GoodWe SEMS monitoring endpoints, including bulk CSV processing for stations.
          </p>
        </div>
      </header>
      ...
      <main className="container mx-auto px-4 py-8 space-y-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>1) Connect GoodWe SEMS</CardTitle>
            <CardDescription>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                <Label htmlFor="goodwe-connection-name">Profile Name (optional)</Label>
                <Input
                  id="goodwe-connection-name"
                  value={connectionNameInput}
                  onChange={(e) => setConnectionNameInput(e.target.value)}
                  placeholder="Example: GoodWe API 1"
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="goodwe-account">Account</Label>
                <Input
                  id="goodwe-account"
                  type="text"
                  value={accountInput}
              ...
              <div className="space-y-2">
                <Label htmlFor="goodwe-password">Password</Label>
                <Input
                  id="goodwe-password"
                  type="password"
                  value={passwordInput}
                      ...
                      <SelectContent>
                        {connections.map((connection) => (
                          <SelectItem key={connection.id} value={connection.id}>
                            {connection.name} ({connection.accountMasked})
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  {connections.map((connection) => (
                    ...
                    >
                      ...
                      <p>Account: {connection.accountMasked}</p>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Upload a CSV of Station IDs and process production snapshots in batches across API profiles.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Button
                ...
                onClick={() => downloadBulkCsv(bulkRows, "goodwe-production-bulk-all")}
              >
                Download All CSV
              </Button>
              <Button
                ...
                onClick={() => downloadBulkCsv(filteredBulkRows, "goodwe-production-bulk-filtered")}
              >
            ...
            <div className="rounded-lg border bg-card p-3 space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                ...
                {bulkConnectionScope === "all"
                  ? `${NUMBER_FORMATTER.format(BULK_BATCH_SIZE_ALL_PROFILES)} stations per request (all API profiles).`
                  : `${NUMBER_FORMATTER.format(BULK_BATCH_SIZE_ACTIVE)} stations per request (active API profile).`}
              </p>
            </div>
