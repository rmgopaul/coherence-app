...
export default function SolisMeterReads() {
  ...
  const stationsQuery = trpc.solis.listStations.useQuery(undefined, {
    enabled: !!user && !!statusQuery.data?.connected,
    retry: false,
  });

  const connectMutation = trpc.solis.connect.useMutation();
  const setActiveConnectionMutation = trpc.solis.setActiveConnection.useMutation();
  const removeConnectionMutation = trpc.solis.removeConnection.useMutation();
  const disconnectMutation = trpc.solis.disconnect.useMutation();
  const productionSnapshotMutation = trpc.solis.getProductionSnapshot.useMutation();
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      await trpcUtils.solis.getStatus.invalidate();
      await trpcUtils.solis.listStations.invalidate();
      setSelectedConnectionId(response.activeConnectionId);
      setApiKeyInput("");
      setApiSecretInput("");
      setConnectionNameInput("");
      toast.success(
        `Solis profile saved. ${NUMBER_FORMATTER.format(response.totalConnections)} profile(s) stored.`
      );
    } catch (error) {
  ...
  const handleSetActiveConnection = async () => {
    ...
    try {
      ...
      toast.success("Active Solis API profile updated.");
    } catch (error) {
    ...
    try {
      ...
      toast.success(
        response.connected
          ...
          : "Removed final profile. Solis is now disconnected."
      );
    } catch (error) {
  ...
  const handleDisconnect = async () => {
    try {
      ...
      setSelectedStationId("");
      setSelectedConnectionId("");
      toast.success("Solis disconnected.");
    } catch (error) {
  ...
  const handlePullAllSites = async () => {
    if (!isConnected) {
      toast.error("Connect Solis before pulling sites.");
      return;
    }
    try {
  ...
  const runBulkSnapshot = async () => {
    if (!statusQuery.data?.connected) {
      toast.error("Connect Solis before running bulk processing.");
      return;
    }
      ...
      if (bulkCancelRef.current) {
        ...
        try {
          const readRows = collectedRows
            ...
            .map((row) =>
              buildConvertedReadRow("Solis", row.stationId, row.name ?? "", row.lifetimeKwh!, row.anchorDate!)
            );
          ...
          if (result.pushed > 0) {
            toast.success(`Pushed ${NUMBER_FORMATTER.format(result.pushed)} Solis rows to Solar REC Dashboard Converted Reads.${result.skipped > 0 ? ` ${NUMBER_FORMATTER.format(result.skipped)} duplicates skipped.` : ""}`);
          } else if (result.skipped > 0) {
            toast.message(`All ${NUMBER_FORMATTER.format(result.skipped)} Solis Converted Reads rows already exist. No new rows pushed.`);
          }
        } catch (pushError) {
  ...
  const downloadConvertedReadsCsv = (rows: BulkSnapshotRow[]) => {
    ...
    const csvRows = readRows.map((row) => {
      const readRow = buildConvertedReadRow("Solis", row.stationId, row.name ?? "", row.lifetimeKwh!, row.anchorDate!);
    ...
    const csvText = buildCsv(headers, csvRows);
    const fileName = `solis-converted-reads-${new Date().toISOString().slice(0, 19).replace(/[:T]/g, "-")}.csv`;
    downloadTextFile(fileName, csvText, "text/csv;charset=utf-8");
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-accent/20">
      <header className="border-b bg-card/90 backdrop-blur-sm sticky top-0 z-10">
        <div className="container mx-auto px-4 py-4">
          ...
          <h1 className="text-2xl font-bold text-foreground">Solis Cloud API</h1>
          <p className="text-sm text-muted-foreground mt-1">
            API Key + Secret connection for Solis Cloud monitoring endpoints, including bulk CSV processing for stations.
          </p>
        </div>
      </header>
      ...
      <main className="container mx-auto px-4 py-8 space-y-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>1) Connect Solis Cloud</CardTitle>
            <CardDescription>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                <Label htmlFor="solis-connection-name">Profile Name (optional)</Label>
                <Input
                  id="solis-connection-name"
                  value={connectionNameInput}
                  onChange={(e) => setConnectionNameInput(e.target.value)}
                  placeholder="Example: Solis API 1"
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="solis-api-key">API Key</Label>
                <Input
                  id="solis-api-key"
                  type="password"
                  value={apiKeyInput}
              ...
              <div className="space-y-2">
                <Label htmlFor="solis-api-secret">API Secret</Label>
                <Input
                  id="solis-api-secret"
                  type="password"
          ...
          <CardContent className="space-y-4">
            ...
            {!stationsQuery.isLoading && !stationsError && isConnected && stations.length === 0 && (
              <div className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-700 dark:border-amber-700 dark:bg-amber-950 dark:text-amber-300">
                No stations were returned for this API key.
              </div>
            )}
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Button
                ...
                onClick={() => downloadBulkCsv(bulkRows, "solis-production-bulk-all")}
              >
                Download All CSV
              </Button>
              <Button
                ...
                onClick={() => downloadBulkCsv(filteredBulkRows, "solis-production-bulk-filtered")}
              >
