...
type SingleOperation = "listSites" | "getProductionSnapshot";
...
function extractSiteIdsFromCsv(text: string): string[] {
  ...
  const preferredIndex = normalizedHeaders.findIndex((header) =>
    ["siteid", "site_id", "id"].includes(header)
  );
...
export default function LocusMeterReads() {
  ...
  const [selectedOperation, setSelectedOperation] = useState<SingleOperation>("listSites");
  ...
  const sitesQuery = trpc.locus.listSites.useQuery(undefined, {
    enabled: !!user && !!statusQuery.data?.connected,
    retry: false,
  });

  const connectMutation = trpc.locus.connect.useMutation();
  const setActiveConnectionMutation = trpc.locus.setActiveConnection.useMutation();
  const removeConnectionMutation = trpc.locus.removeConnection.useMutation();
  const disconnectMutation = trpc.locus.disconnect.useMutation();
  const productionSnapshotMutation = trpc.locus.getProductionSnapshot.useMutation();
  ...
  const handleConnect = async () => {
    ...
    if (!clientId || !clientSecret || !partnerId) {
      toast.error("Enter Client ID, Client Secret, and Partner ID.");
      return;
    }
    ...
    try {
      ...
      await trpcUtils.locus.getStatus.invalidate();
      await trpcUtils.locus.listSites.invalidate();
      ...
      toast.success(
        `Locus profile saved. ${NUMBER_FORMATTER.format(response.totalConnections)} profile(s) stored.`
      );
    } catch (error) {
  ...
  const handleSetActiveConnection = async () => {
    ...
    try {
      ...
      await trpcUtils.locus.listSites.invalidate();
      setSelectedSiteId("");
      toast.success("Active Locus API profile updated.");
    } catch (error) {
    ...
    try {
      ...
      toast.success(
        response.connected
          ...
          : "Removed final profile. Locus is now disconnected."
      );
    } catch (error) {
  ...
  const handleDisconnect = async () => {
    try {
      ...
      setSelectedSiteId("");
      setSelectedConnectionId("");
      toast.success("Locus disconnected.");
    } catch (error) {
  ...
  const handleRunSingleOperation = () => {
    switch (selectedOperation) {
      case "listSites":
        void runAction("List Sites", () =>
          sitesQuery.refetch().then((result) => result.data)
        );
        break;
  ...
  const handlePullAllSites = async () => {
    if (!isConnected) {
      toast.error("Connect Locus before pulling sites.");
      return;
    }
    try {
      const result = await sitesQuery.refetch();
      const sites = result.data?.sites ?? [];
      if (sites.length === 0) {
        toast.error("No sites found for this API profile.");
        return;
      }
      const ids = sites.map((s: { siteId: string }) => s.siteId);
      setBulkSiteIds(ids);
      setBulkSourceFileName(`API — ${ids.length} sites`);
      setBulkRows([]);
      setBulkImportError(null);
      setBulkProgress({ total: ids.length, processed: 0, found: 0, notFound: 0, errored: 0 });
      toast.success(
        `Loaded ${NUMBER_FORMATTER.format(ids.length)} Site IDs. Next step: click "Run Production Snapshot" to fetch row data.`
      );
    } catch (error) {
      toast.error(`Failed to list sites: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  };

  const runBulkSnapshot = async () => {
    if (!statusQuery.data?.connected) {
      toast.error("Connect Locus before running bulk processing.");
      return;
    }
    ...
    try {
      ...
      if (bulkCancelRef.current) {
        toast.message(
          `Stopped production snapshots after ${NUMBER_FORMATTER.format(processed)} of ${NUMBER_FORMATTER.format(bulkSiteIds.length)} Site IDs.`
        );
      } else {
        toast.success(
          `Completed production snapshots for ${NUMBER_FORMATTER.format(processed)} Site IDs using ${bulkConnectionScope === "all" ? "all saved API profiles" : "active API profile"}. Found ${NUMBER_FORMATTER.format(found)}, not found ${NUMBER_FORMATTER.format(notFound)}, errors ${NUMBER_FORMATTER.format(errored)}.`
        );
        ...
        try {
          const readRows = collectedRows
            ...
            .map((row) =>
              buildConvertedReadRow("Locus Energy", row.siteId, row.name ?? "", row.lifetimeKwh!, row.anchorDate!)
            );
          ...
          if (result.pushed > 0) {
            toast.success(`Pushed ${NUMBER_FORMATTER.format(result.pushed)} Locus rows to Solar REC Dashboard Converted Reads.${result.skipped > 0 ? ` ${NUMBER_FORMATTER.format(result.skipped)} duplicates skipped.` : ""}`);
          } else if (result.skipped > 0) {
            toast.message(`All ${NUMBER_FORMATTER.format(result.skipped)} Locus Converted Reads rows already exist. No new rows pushed.`);
          }
        } catch (pushError) {
  ...
  const filteredBulkRows = useMemo(() => {
    ...
    const filtered = bulkRows.filter((row) => {
      ...
      const haystack = `${row.siteId} ${row.name ?? ""} ${row.status} ${row.error ?? ""}`.toLowerCase();
  ...
  const downloadConvertedReadsCsv = (rows: BulkSnapshotRow[]) => {
    ...
    const csvRows = readRows.map((row) => {
      const readRow = buildConvertedReadRow("Locus Energy", row.siteId, row.name ?? "", row.lifetimeKwh!, row.anchorDate!);
    ...
    const csvText = buildCsv(headers, csvRows);
    const fileName = `locus-converted-reads-${new Date().toISOString().slice(0, 19).replace(/[:T]/g, "-")}.csv`;
    downloadTextFile(fileName, csvText, "text/csv;charset=utf-8");
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-accent/20">
      <header className="border-b bg-card/90 backdrop-blur-sm sticky top-0 z-10">
        <div className="container mx-auto px-4 py-4">
          ...
          <h1 className="text-2xl font-bold text-foreground">Locus Energy API</h1>
          <p className="text-sm text-muted-foreground mt-1">
            OAuth connection for Locus Energy monitoring endpoints, including bulk CSV processing for thousands of sites.
          </p>
        </div>
      </header>
      ...
      <main className="container mx-auto px-4 py-8 space-y-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>1) Connect Locus Energy</CardTitle>
            <CardDescription>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="space-y-2">
                <Label htmlFor="locus-connection-name">Profile Name (optional)</Label>
                <Input
                  id="locus-connection-name"
                  value={connectionNameInput}
                  onChange={(e) => setConnectionNameInput(e.target.value)}
                  placeholder="Example: Locus API 1"
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="locus-client-id">Client ID</Label>
                <Input
                  id="locus-client-id"
                  type="password"
                  value={clientIdInput}
              ...
              <div className="space-y-2">
                <Label htmlFor="locus-client-secret">Client Secret</Label>
                <Input
                  id="locus-client-secret"
                  type="password"
              ...
              <div className="space-y-2">
                <Label htmlFor="locus-partner-id">Partner ID</Label>
                <Input
                  id="locus-partner-id"
                  type="password"
        ...
        {/* Section 2: Single Site API Tester */}
        <Card>
          <CardHeader>
            <CardTitle>2) Single Site API Tester</CardTitle>
            <CardDescription>
              Pick a site from the list or paste one manually, then fetch endpoint responses.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2 md:col-span-1">
                ...
                <Select value={selectedSiteId} onValueChange={setSelectedSiteId} disabled={!sites.length}>
                  ...
                  <SelectContent>
                    {sites.map((site: { siteId: string; name?: string | null }) => (
                      <SelectItem key={site.siteId} value={site.siteId}>
                        {site.name ?? site.siteId} ({site.siteId.slice(0, 8)}...)
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              <div className="space-y-2">
                ...
                <Select value={selectedOperation} onValueChange={(value) => setSelectedOperation(value as SingleOperation)}>
                  ...
                  <SelectContent>
                    <SelectItem value="listSites">List Sites</SelectItem>
            ...
            {sitesQuery.isLoading && <div className="text-sm text-muted-foreground">Loading sites...</div>}
            ...
            {!sitesQuery.isLoading && !sitesError && isConnected && sites.length === 0 && (
              <div className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-700 dark:border-amber-700 dark:bg-amber-950 dark:text-amber-300">
                No sites were returned for this API profile.
              </div>
            )}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Upload a CSV of Site IDs and process production snapshots in batches across API profiles.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-2 md:col-span-1">
                ...
                <div className="flex items-center gap-2">
                  <Button
                    ...
                    disabled={!isConnected || sitesQuery.isFetching}
                ...
                <p className="text-xs text-muted-foreground">
                  Source: {bulkSourceFileName ?? "None"}{bulkSiteIds.length > 0 ? ` — ${NUMBER_FORMATTER.format(bulkSiteIds.length)} IDs` : ""}
                </p>
                <p className="text-xs text-muted-foreground">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Button
                ...
                onClick={() => downloadBulkCsv(bulkRows, "locus-production-bulk-all")}
              >
                Download All CSV
              </Button>
              <Button
                ...
                onClick={() => downloadBulkCsv(filteredBulkRows, "locus-production-bulk-filtered")}
              >
            ...
            <div className="grid gap-3 md:grid-cols-5">
              <div className="rounded-lg border bg-card p-3">
                <p className="text-xs text-muted-foreground">Imported Site IDs</p>
                <p className="text-xl font-semibold text-foreground">{NUMBER_FORMATTER.format(bulkSiteIds.length)}</p>
              </div>
              <div className="rounded-lg border bg-card p-3">
              ...
              <TableBody>
                {bulkPageRows.map((row) => (
                  <TableRow key={row.siteId}>
                    <TableCell className="font-medium font-mono text-xs">{row.siteId}</TableCell>
